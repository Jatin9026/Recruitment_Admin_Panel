...
const DomainInterviewBase = ({ domain }) => {
  ...
  useEffect(() => {
    let filtered = applicants.filter(applicant => {
      ...
      // Status filter based on PI entry for current domain
                              ...
                              <div className="space-y-1">
                                ...
                                {applicant.domains && applicant.domains.length > 0 && (
                                  <div className="text-xs text-blue-600">
                                    <span className="font-medium">Domains:</span> {applicant.domains.join(', ')}
                                  </div>
                                )}
                              </div>
                            </div>
          ...
          >
            ...
            <div className="flex-1 p-6 overflow-y-auto min-h-0">
              ...
              <div className="bg-gray-50 rounded-xl p-4 mb-6">
                ...
                {/* Screening Domains */}
                {evaluatingApplicant.domains && evaluatingApplicant.domains.length > 0 && (
                  <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                    <div className="flex items-center mb-2">
                      <Target className="w-4 h-4 text-green-600 mr-2" />
                      <span className="font-medium text-green-800 text-sm">Screening Domains</span>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {evaluatingApplicant.domains.map((domain, index) => (
                        <span key={index} className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-green-100 text-green-800 border border-green-300">
                          {domain}
                        </span>
                      ))}
                    </div>
                    <p className="text-xs text-green-600 mt-2">
                      Domains selected during screening evaluation
                    </p>
                  </div>
                )}
