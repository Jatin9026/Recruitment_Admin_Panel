...
const DomainInterview = () => {
  ...
  if (loading) return <p>Loading interview applicants...</p>;

  return (
    <div className="p-4">
      <h1 className="text-xl font-semibold mb-4">Domain Interviews</h1>
      {applicants.length === 0 ? (
        <p>No applicants scheduled for domain interviews.</p>
      ) : (
          ...
          <tbody>
            {applicants.map((applicant) => (
              <tr key={applicant.id} className="hover:bg-gray-50">
                ...
                <td className="p-2 border">{applicant.group}</td>
                <td className="p-2 border">
                  {/* fallback if interviewDate not present */}
                ...
                <td className="p-2 border">
                  <Link
                    to={`/interview/evaluate/${applicant.id}`}
