...
export default function VerifiedPayments() {
  ...
  const fetchOrders = async ({ showLoader = true, upiId = selectedUpiId } = {}) => {
    try {
      ...
      const response = await endeavourApiClient.getOrdersVerification(params);
      const list = response?.data || response?.orders || [];
      setOrders(Array.isArray(list) ? list : []);
    } catch (err) {
      setError(err?.message || "Unable to load verified orders.");
      setOrders([]);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      if (!e.target.closest("#verified-upi-dropdown-root")) {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <p className="text-lg font-medium text-slate-700">Loading verified payments...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-semibold text-slate-900 sm:text-3xl">Verified Payments</h1>
                <p className="mt-1 text-sm text-slate-600">
                  Orders with verification status <span className="font-medium text-emerald-700">approved</span>
                  {selectedUpiId ? (
                    <>
                      {" "}
                      for <span className="font-mono font-medium">{selectedUpiId}</span>
                    </>
                  ) : (
                    <> (all UPI accounts)</>
                  )}
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div id="verified-upi-dropdown-root" className="relative">
                <button
                  type="button"
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 px-5 py-4 sm:px-6">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-lg font-semibold text-slate-900">Verified orders</h2>
              ...
              <div className="text-sm text-slate-500">
                GET /admin/orders/verification?status=approved
                {selectedUpiId ? "&upi_id=…" : ""}
              </div>
            </div>
          </div>
                ...
                {filteredOrders.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-16 text-center">
                      ...
                      <h3 className="text-base font-medium text-slate-900">No verified orders found</h3>
                      <p className="mt-1 text-sm text-slate-500">
                        Try another UPI filter, clear search, or refresh.
                      </p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={order.id} className="hover:bg-slate-50/70">
                      ...
                      <Td>
                        <div className="space-y-2">
                          <Badge tone="blue">{order.verification_status || "approved"}</Badge>
