...
export default function LoginPage({ fixedTab = null, allowTabSwitch = true }) {
  const initialTab = fixedTab === "ideatex" ? "ideatex" : "recruitment";
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      if (activeTab === "ideatex") {
        navigate(IDEATEX_PATHS.root);
      } else {
        navigate(RECRUITMENT_PATHS.root);
      }
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50 flex items-center justify-center font-caldina p-4">
      ...
      >
        ...
        {allowTabSwitch ? (
          ...
          <div className="mb-6 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-center text-sm font-semibold text-gray-700">
            Event Module: {activeTab === "ideatex" ? "Ideatex" : "Recruitment"}
          </div>
        )}
